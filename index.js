...
server.tool(
  ...
  'One-call per-device circuit/interface status summary. Use for Korean requests like "장비별 회선현황", "전체 장비 포트 현황", or "site/group device interface summary"; avoids list_devices + repeated get_device_interfaces calls.',
  {
    ...
    group_id:       z.number().optional().describe('Device group ID filter'),
    disabled:       z.number().optional().describe('Device disabled filter (0=enabled, 1=disabled)'),
    admin_status:   z.number().optional().describe('Interface admin status filter (1=UP, 2=DOWN)'),
    oper_status:    z.number().optional().describe('Interface operational status filter (1=UP, 2=DOWN)'),
    limit:          z.number().optional().default(100).describe('Maximum devices to return, default 100'),
  },
  async (args) => {
...
// ════════════════════════════════════════════════════════
// 서버 시작
// 실행 모드: --http [포트]  → HTTP (SSE + StreamableHTTP)
//            (기본)         → stdio
// ════════════════════════════════════════════════════════
...
async function startHttp(port) {
  ...
  // StreamableHTTP: 요청이 들어올 때 transport 연결
  // (미리 connect()하면 active request 없이 즉시 닫힘)
  let transport = null;
  ...
  app.all('/mcp', async (req, res) => {
    ...
    const sessionHdr = req.headers['mcp-session-id'] ?? '-';
    httpLog(
      `→ ${req.method} /mcp | from=${remote} | Accept=${accept} | Content-Type=${ctype} | Mcp-Session-Id=${sessionHdr}`
    );
    if (req.method === 'POST' && req.body != null && typeof req.body === 'object') {
      const raw = JSON.stringify(req.body);
      const preview = raw.length > 400 ? `${raw.slice(0, 400)}…` : raw;
      httpLog(`  JSON-RPC body: ${preview}`);
    }
    res.on('finish', () => {
      httpLog(`← ${req.method} /mcp done status=${res.statusCode}`);
    });
    try {
      if (!transport) {
        httpLog('StreamableHTTPServerTransport 생성 및 server.connect()');
        transport = new StreamableHTTPServerTransport({
          sessionIdGenerator: () => Math.random().toString(36).slice(2),
        });
        transport.onerror = (err) => {
          httpLog(`transport onerror: ${err?.message ?? err}`);
        };
        transport.onclose = () => {
          httpLog('transport onclose → 세션 해제');
          transport = null;
        };
        await server.connect(transport);
      }
  ...
  app.get('/health', (req, res) => {
    httpLog(`GET /health from ${req.socket?.remoteAddress ?? req.ip ?? '-'}`);
    res.json({ status: 'ok', server: 'xv3-nms-mcp' });
  });
  ...
  app.listen(port, () => {
    process.stderr.write(
      ...
      `  (브라우저로 /mcp 는 MCP 전용이라 JSON 오류가 나는 것이 정상입니다. 로그로 요청 여부를 확인하세요.)\n`
    );
  });
}
