...
| 42 | `list_events` | 이벤트 | 이벤트 로그 조회 (Syslog/Threshold/Ping 등, xv3_issue DB) |
| 43 | `get_event_summary` | 이벤트 | 이벤트 유형별·심각도별 집계 요약 |
| 44 | `list_persistent_issues` | 이벤트 | 장기 지속 이슈 조회 (현재 활성 이슈 기본) |
...
> 이벤트 데이터는 `xv3_issue` DB의 `issue_log` / `issue_log_persist` 테이블에서 조회합니다.

### issue_type 코드 참고

| 코드 | 이름 | 설명 |
|------|------|------|
| S | Syslog | 장비 Syslog 메시지 |
| C | Threshold Event | 임계값 위반 이벤트 |
| P | Ping | Ping 상태 변경 (UP/DOWN) |
| A | Temp. Fault | 온도 장애 |
| F | FAN Fault | 팬 장애 |
| I | Port Status | 포트 상태 변경 |
| M | System Event | 시스템 이벤트 |
| N | Module Down | 모듈 다운 |
| E | EventManager | 이벤트 매니저 |
| O | AutoProvisioning | 자동 프로비저닝 |
| R | Power Fault | 전원 장애 |
| W | Wireless Threshold Event | 무선 임계값 이벤트 |

### syslog_keyword 형식

Cisco IOS Facility-Severity 코드 형식입니다 (예: `5-4`).
- 앞자리: Facility (5=Notification 등)
- 뒷자리: Severity (3=Error, 4=Warning, 5=Notice 등)

---

### `list_events`

xv3_issue DB의 이벤트 로그를 조회합니다. Syslog, Threshold, Ping 등 모든 이벤트 유형을 포함합니다.

| 파라미터 | 필수 | 설명 |
|----------|------|------|
| `device_id` | 선택 | 장비 ID로 필터 |
| `site_id` | 선택 | 사이트 ID로 필터 |
| `issue_type` | 선택 | 이벤트 유형 코드 (S/C/P/A/F/I/M/N/E/O/R/W) |
| `severity` | 선택 | 심각도 (1=Critical, 2=Major, 3=Minor, 4=Warning, 5=Normal) |
| `syslog_keyword` | 선택 | Facility-Severity 코드 부분 검색 (예: 5-4) |
| `start_date` | 선택 | 조회 시작 일시 (예: 2026-05-01 00:00:00) |
| `end_date` | 선택 | 조회 종료 일시 |
| `active_only` | 선택 | 미종료 이벤트만 조회 (true/false) |
| `limit` | 선택 | 최대 건수 (기본 100) |
...
이벤트를 이벤트 유형별·심각도별로 집계합니다. 결과는 `{ total, by_type: [...], by_severity: [...] }` 형태입니다.

| 파라미터 | 필수 | 설명 |
|----------|------|------|
| `device_id` | 선택 | 특정 장비로 범위 제한 |
| `site_id` | 선택 | 특정 사이트로 범위 제한 |
| `start_date` | 선택 | 집계 시작 일시 |
| `end_date` | 선택 | 집계 종료 일시 |
...
|----------|------|------|
| `device_id` | 선택 | 장비 ID로 필터 |
| `issue_type` | 선택 | 이벤트 유형 코드 (S/C/P 등) |
| `severity` | 선택 | 심각도 (1=Critical, 2=Major, 3=Minor, 4=Warning, 5=Normal) |
| `active_only` | 선택 | 미종료만 조회 (기본 true) |
| `limit` | 선택 | 최대 건수 (기본 100) |
